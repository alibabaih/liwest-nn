...
// 					$date = JFactory::getDate()->toMySQL();
	 ...
	 *
	 * @author RolandD, Christopher Roussel, ValГ©rie Isaksen, Max Milbers
	 *
	 */
...
if (isset($vars['newOrderData']['current_order_status'])) {
    ...
    $adminMsg = "Статус заказа {$orderNum} изменен на \"{$newStatus}\"";    $userMsg = "{$firstName}, статус Вашего заказа {$orderNum} изменен на \"{$newStatus}\"";} else {
    $adminMsg = "Поступил новый заказ {$orderNum} от {$firstName} {$lastName}";    $userMsg = "Ваш заказ {$orderNum} поступил в обработку. В скором времени наши менеджеры с вами свяжутся!";}

// send sms to admin
ByteHandApi::sendSms(array('89049239309'), $adminMsg);
// укажите ваш номер
// send sms to user
