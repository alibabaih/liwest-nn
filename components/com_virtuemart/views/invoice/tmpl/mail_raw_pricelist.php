...
// Head of table
echo strip_tags(JText::sprintf('Заказанные товары', $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_total,$this->currency))) . "\n";
echo sprintf("%'-64.64s", '') . "\n";
//echo JText::_('COM_VIRTUEMART_ORDER_ITEM') . "\n";
foreach ($this->orderDetails['items'] as $item) {
    echo "\n";
    //echo $item->order_item_name . ' (' . strtoupper(JText::_('Артикул: ')) . $item->order_item_sku . ')' ."—". . $item->product_quantity . 'шт.  ';
    ...
    echo JText::_('Итого на сумму: ') . $item->product_final_price;
...
// Coupon не забыть оттестировать!!!!
...
echo strtoupper(JText::_('Выбранный способ доставки: ')) . ' (' . strip_tags(str_replace("<br />", "\n", $this->orderDetails['shipmentName'])) . ' ) ' . "\n";
...
echo "\n";
echo strtoupper(JText::_('Выбранный способ оплаты: ')) . ' (' . strip_tags(str_replace("<br />", "\n", $this->orderDetails['paymentName'])) . ' ) ' . "\n";
...
echo strtoupper(JText::_('Итоговая суммма с учётом выбранных условий доставки и оплаты: ')) . ' : ' . $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_total,$this->currency) . "\n";
