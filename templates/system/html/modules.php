...
/*
 * xhtml (divs and font headder tags) можернизированный, оригинальный чуть ниже
 */
function modChrome_xhtml($module, &$params, &$attribs)
{
	if (!empty ($module->content)) : ?>
		...
		<?php if ($module->showtitle != 0) : ?>
			<div class="block-name"><h1 class="text-center"><?php echo $module->title; ?></h1></div>
		<?php endif; ?>
			<?php echo $module->content; ?>
		</div>
	<?php endif;
...
function modChrome_news($module, &$params, &$attribs)
{
    if (!empty ($module->content)) : ?>
        <div class="blog-post big">
            <?php if ($module->showtitle != 0) : ?>
                <h3 class="no-margin-top"><?php echo $module->title; ?></h3>
            <?php endif; ?>
            <?php echo $module->content; ?>
        </div>
    <?php endif;
    ...
    if (!empty ($module->content)) : ?>
        ...
        <div class="owl-carousel-container">
            <div class="owl-header">
                <h3 class="animate-onscroll"><?php echo $module->title; ?></h3>
...
/*
 * xhtml (divs and font headder tags) можернизированный, оригинальный чуть ниже, для блока новостей, там ссылка стоит
 */
function modChrome_cleansale($module, &$params, &$attribs)
{
    if (!empty ($module->content)) : ?>
        <?php if ($module->showtitle != 0) : ?>
            <div class="block-name"><h1 class="text-center"><a href="/sale">Акции</a></h1></div>
        <?php endif; ?>
        <?php echo $module->content; ?>
    <?php endif;
}
