...
echo strip_tags(JText::sprintf('Заказ сформирован (Ли Вест НН)')) . "\n";
...
echo JText::sprintf('COM_VIRTUEMART_MAIL_SHOPPER_CONTENT', 
	...
	$this->orderDetails['details']['BT']->order_pass, 
	$this->orderDetails['details']['BT']->created_on);

echo "\n" . strip_tags(JText::sprintf('Состояние заказа', $this->orderDetails['details']['BT']->order_status_name)) . ".";
...
if ($this->orderDetails['history'][$nb - 1]->customer_notified && !(empty($this->orderDetails['history'][$nb - 1]->comments))) {
    ...
    echo JText::_('Вы можете просмотреть заказ по адресу: ') . JURI::root() . 'index.php?option=com_virtuemart&view=orders&layout=details&order_number=' . $this->orderDetails['details']['BT']->order_number . '&order_pass=' . $this->orderDetails['details']['BT']->order_pass . "\n";
